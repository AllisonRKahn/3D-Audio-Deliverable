...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=None, tty="/dev/tty.usbmodem1421", baud=9600, maxvaluescount=3, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        self.tty = tty  # serial
    ...
    # this is the function that I created to send the updated values
    def send_position(self):
        if(isinstance(self.position()[1],int)):
            return [self.position()[0], self.position()[1], self.position()[2], self.position()[3], self.position()[4], self.position()[5]/1000.0]
        else:
            return [self.position()[0], self.position()[1], self.position()[2], self.position()[3], self.position()[4], self.position()[5]/1000.0]
