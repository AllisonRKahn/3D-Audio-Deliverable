...
if __name__ == '__main__':
    increments = range(0,len(SIG_DATA)//BUFFER_SIZE)
    index      = 0
    a_index    = 0
    ...
    while index < len(increments):
        ...
        left  = np.squeeze(DB_DATA['hrir_l'][a_index, E_INDEX, :])
        right = np.squeeze(DB_DATA['hrir_r'][a_index, E_INDEX, :])
        delay = DB_DATA['ITD'][a_index, E_INDEX]
        zeros = np.zeros(int(math.floor(abs(delay))))
